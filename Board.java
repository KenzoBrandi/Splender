...
public class Board implements Displayable {
    ...
    public Board(int nbPlayers){
        ...
        if (nbPlayers == 2){
            ...
            resources.forEach((key, value) ->{
                resources.put(key,new Integer(7));
            });
        }
    ...
    public DevCard drawCard(int niveau){
        Stack<DevCard> pile = stackCards.get(3-niveau);
    ...
    public boolean canGiveSameTokens(Resource type){
        int i = resources.getNbResource(type);
        return i >= 4;
    }
    
    /**
     * Retourne s'il est possible de prendre un jeton du type donné
     * Input: Resource type du jeton
     * Output: Boolean
     */
